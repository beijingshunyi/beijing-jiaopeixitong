...
async function createPayment(req, res) {
  try {
    const { student_id, course_id, amount, payment_type, payment_method, notes } = req.body;
...
async function getPayments(req, res) {
  try {
    const { student_id, course_id, status, start_date, end_date, page = 1, limit = 20 } = req.query;
...
async function processRefund(req, res) {
  try {
    const { payment_id, amount, reason, refund_method } = req.body;
